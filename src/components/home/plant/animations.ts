...
export const STYLE = [
  ...
  /* travel distance comes in via --plt-scan-dist (see geometry.SCAN_DIST); the */
  /* px fallback below only applies if that var is somehow never supplied */
  ...
  ".plt-led{animation:plt-led 1.1s steps(2) infinite;animation-delay:var(--plt-dl,0s)}",
  ".plt-reticle{animation:plt-reticle 3.2s ease-in-out infinite}",
  ".plt-callout{animation:plt-callout 5s ease-in-out infinite;animation-delay:var(--plt-dl,0s);transform-box:fill-box;transform-origin:center}",
  ".plt-tick{animation:plt-tick 6.4s ease-in-out infinite}",
  /* freeze while scrolled out of view (toggled by IntersectionObserver) */
  ".plt-paused *{animation-play-state:paused!important}",
  "@media (prefers-reduced-motion: reduce){" +
    ".plt-leaf,.plt-breath,.plt-spore,.plt-scanline,.plt-ring,.plt-bloom,.plt-pulse,.plt-reticle,.plt-callout,.plt-tick{animation:none!important}" +
    ".plt-plaque,.plt-led{animation:none!important;opacity:.85}" +
    "}",
  ...
  "@keyframes plt-scan{0%,100%{transform:translateY(var(--plt-scan-dist,-102px));opacity:.7}50%{transform:translateY(0);opacity:.95}}",
  ...
  "@keyframes plt-led{0%,100%{opacity:.18}50%{opacity:1}}",
  "@keyframes plt-reticle{0%,100%{opacity:.35}50%{opacity:.85}}",
  "@keyframes plt-callout{0%,100%{opacity:.55}50%{opacity:1}}",
  "@keyframes plt-tick{0%,100%{opacity:.3}50%{opacity:.65}}",
].join("");